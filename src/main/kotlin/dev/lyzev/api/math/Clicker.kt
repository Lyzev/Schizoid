...
class Clicker(val container: IFeature) {
    ...
    val cooldownThreshold by container.slider(
        "Cooldown Threshold",
        "The threshold for the cooldown.",
        90,
        0,
        100,
        "%%",
        hide = ::type neq Type.Cooldown
    )
    val cps by container.slider(
        "CPS",
        "The clicks per second of the clicker.",
        16,
        1,
        20,
        "cps",
        hide = ::type eq Type.Cooldown
    )
    ...
    fun tick(force: Boolean): Int {
        ...
        when (type) {
            ...
            Type.Cooldown -> {
                if (mc.attackCooldown <= 0 && mc.player!!.getAttackCooldownProgress(1f) >= cooldownThreshold / 100f) {
                    clicks[System.currentTimeMillis()] = 1
                    return 1
                }
                return 0
